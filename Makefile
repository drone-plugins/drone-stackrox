...
CONTAINER_REPO        ?=  plugins/stackrox
...
stackrox: ## Runs stackrox target
stackrox: vendor main.go $(wildcard *.go) $(wildcard */*.go) ; $(info $(M) running stackrox )
...
tmp/help.txt: stackrox
	-mkdir -p tmp
	$(ROOT_DIR)/stackrox --help &> tmp/help.txt
...
compress: stackrox ; $(info $(M) running compress )
	# Add as dependency
	$(Q) $(UPX) stackrox

.PHONY: container
container: ## Builds stackrox docker image with latest tag
container: stackrox Dockerfile ; $(info $(M) running container )
